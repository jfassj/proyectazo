...
const alumnoEliminadot = () => {
  toast.warn('Alumno eliminado!', {
    position: "top-right",
    autoClose: 5000,
...
function AlumnoEliminart() {
  ...
  const handleSubmitt = (event) => {
    ...
    deleteAlumnot(alumno);
  };

  const deleteAlumnot = async (data) => {
    const response = await axios.post(
      "http://localhost:5000/alumnot/eliminar",
      data
    );
    if (response.status === 200) {
    ...
    <>
      <Container>
        ...
        <Form onSubmit={handleSubmitt}>
          ...
          <Row>
            <Col>
              <Button onClick={alumnoEliminadot} type="submit" className="btn btn-danger">
                Eliminar
              </Button>
            </Col>
...
export default AlumnoEliminart;
